...
function platoon_connected(; deterministic_switching::Bool=true,
    ...
    if deterministic_switching
        invariant = HalfSpace(SingleEntryVector(n, n, 1.), c1)  # t <= c1
    else
...
function platoon_disconnected(; deterministic_switching::Bool=true,
    ...
    if deterministic_switching
        invariant = HalfSpace(SingleEntryVector(n, n, 1.), c2)  # t <= c2
    else
        invariant = Universe(n)
    end
...
function platoon(; deterministic_switching::Bool=true,
    ...
    # transition l1 -> l2
    if deterministic_switching
        guard = Hyperplane(SingleEntryVector(n, n, 1.), c1)  # t == c1
    else
        # tb <= t <= tc
        guard = HPolyhedron([HalfSpace(SingleEntryVector(n, n, -1.), -tb),
                             HalfSpace(SingleEntryVector(n, n, 1.), tc)])
    end
    t1 = ConstrainedResetMap(n, guard, reset)

    # transition l2 -> l1
    if deterministic_switching
        guard = Hyperplane(SingleEntryVector(n, n, 1.), c2)  # t == c2
    else
        guard = HalfSpace(SingleEntryVector(n, n, 1.), tr)  # t <= tr
    end
