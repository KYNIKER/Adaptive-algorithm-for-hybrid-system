...
function loadSpacecraft(; abort_time::Union{Float64, Vector{Float64}}=-1.)
    ...
    invariant = HPolyhedron([LazySets.HalfSpace(SingleEntryVector(x, n, 1.), -100.)])  # x <= -100
    if aborting
    ...
    loc3Constraint = [
        # target_avoidance property
        HPolyhedron([LazySets.HalfSpace(sparsevec([x], [1.], n), 0.2), 
                    LazySets.HalfSpace(sparsevec([x], [-1.], n), -0.2),
                    LazySets.HalfSpace(sparsevec([y], [1.], n), 0.2),
                    LazySets.HalfSpace(sparsevec([y], [-1.], n), -0.2)])
    ]
    ...
    if aborting
        push!(properties, LazySets.HalfSpace(sparsevec([t], [1.], n), t_abort_upper))  # t <= t_abort_upper
    end
