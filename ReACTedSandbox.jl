...
dirs = [] # Which direction to plot. Empty means no plotting
...
res = ReACTed(sys, initialState, [0., T], X0, Zonotope(zeros(Float64, n), zeros(Float64, n, 1)), dirs, sys.globalConstraints, δ⁻, δ⁺, ReachabilityAnalysis.Exponentiation.BaseExp, 5, 5; clustering=false, mustSemantics=false)
...
plotProjectedFlowpipeLazy(res, dirs, n, joinpath("results/", "Platoon.png"); verbose=true, xlim=(108.0, 110.0), ylim=(2.0, 3.0))#; xlim=(108.0, 110.0), ylim=(2.0, 3.0)
