...
plot!(Z, c=:purple, lab="Original Zonotope", alpha = 0.2)
plot!(inv, c=:yellow, lab="invariant", alpha = 0.2)
flipped_constraint_list = map(x -> LazySets.HalfSpace(-x.a, -x.b), inv.constraints)
plot!(HPolyhedron(flipped_constraint_list), c=:green, lab="bad compliment of invariant", alpha = 0.2)

intersect, rest = splitZonotope(Z, inv)

plot!(intersect, c=:black, lab="Intersect", alpha = 0.5)
plot!(rest, c=:blue, lab="Rest", alpha = 0.5)
