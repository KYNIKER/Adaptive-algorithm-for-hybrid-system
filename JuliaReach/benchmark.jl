...
algCheckDict = Dict(
    ...
    "PLAD01-BND30-discrete" => LGG09(δ=δ_discrete, template=dirs, approx_model=NoBloating()),
    "SRA01" => BOX(δ=0.04),
...
function JuliaReachTest(name) 
    ...
    modelname = caseCheckDict[name]

    if modelname == "embrake" 
        sol, df = runEMBrake(alg, name)
    elseif modelname == "gearbox"
        sol, df = runGearbox(alg, name)
