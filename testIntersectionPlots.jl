...
test = HPolytope([
    LazySets.HalfSpace(sparsevec([1], [2.], 2), 4.6),  # x <= 0.6
    LazySets.HalfSpace(sparsevec([1], [-1.], 2), 1.2),  # x <= 0.6
    LazySets.HalfSpace(sparsevec([2], [1.], 2), 2.6), # y <= 0.6
    LazySets.HalfSpace(sparsevec([2], [-1.], 2), 0.6), # y <= 0.6
    LazySets.HalfSpace(sparsevec([1, 2], [-1., 1.], 2), 0.7), # y <= 0.6
...
A::Matrix = [0.0 -1.0; 1.0 0.0]

#Plots.plot!(mapPolytope(A, approx), c=:pink, lab="A * Intersect2", alpha=0.5)
Plots.plot!(test, c=:blue, lab="Test", alpha=0.5)
#=
intPoint = LazySets.an_element(test)
vsA, psA = vertexRep(test)
@show constraints_list(A * test)
@show vsA, psA
map!(x -> A * x, vsA)
...
Plots.plot!(linear_map(A, test), c=:pink, lab="JReach", alpha=1.0, lw=1.25)
Plots.plot!(mpPol(A, test), c=:black, lab="new method", alpha=0.8, lw=0.5, fillstyle=://)
