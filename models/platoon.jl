...
function platoon_connected(; deterministic_switching::Bool=true,
    ...
    if deterministic_switching
        invariant = sparseHPolyhedronToDense(HPolyhedron([LazySets.HalfSpace(SingleEntryVector(n, n, 1.), c1)]))  # t <= c1
    else
        invariant = nothing
    end
...
function platoon_disconnected(; deterministic_switching::Bool=true,
    ...
    if deterministic_switching
        invariant = sparseHPolyhedronToDense(HPolyhedron([LazySets.HalfSpace(SingleEntryVector(n, n, 1.), c2)]))  # t <= c2
    else
        invariant = nothing
    end
...
function loadPlatoon(; deterministic_switching::Bool=true,
    ...
    # transition l1 -> l2
    if deterministic_switching
        guard = sparseHPolyhedronToDense(HPolyhedron([#LazySets.HalfSpace(SingleEntryVector(n, n, 1.), c1), # This part is enforced by invarients
            LazySets.HalfSpace(SingleEntryVector(n, n, -1.), -c1)]))  # t >= c1
    else
        # tb <= t <= tc
        guard = sparseHPolyhedronToDense(HPolyhedron([LazySets.HalfSpace(SingleEntryVector(n, n, -1.), -tb),
            LazySets.HalfSpace(SingleEntryVector(n, n, 1.), tc)]))
    end
    ...
    # transition l2 -> l1
    if deterministic_switching
        guard = sparseHPolyhedronToDense(HPolyhedron([#LazySets.HalfSpace(SingleEntryVector(n, n, 1.), c2),  # This part is handled by invarients
            LazySets.HalfSpace(SingleEntryVector(n, n, -1.), -c2)]))  # t >= c2
    else
        guard = sparseHPolyhedronToDense(HPolyhedron([LazySets.HalfSpace(SingleEntryVector(n, n, 1.), tr)]))  # t <= tr
    end
